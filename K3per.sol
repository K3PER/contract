...
interface IK3perNetworkHelper {
    function getQuoteLimit(uint gasUsed) external view returns (uint);
}

contract K3perNetwork is ReentrancyGuard {
    using SafeMath for uint;
    using SafeERC20 for IERC20;

    /// @notice K3perNetwork Helper to set max prices for the ecosystem
    IK3perNetworkHelper public KPRH;

    /// @notice EIP-20 token name for this token
    string public constant name = "k3per.com";

    /// @notice EIP-20 token symbol for this token
    string public constant symbol = "K3PER";
    ...
    uint public constant initialSupply = 100001 * 10 ** 18;
    ...
    function applyCreditToJob(address provider, address liquidity, address job) external {
        ...
        uint _liquidity = K3perNetworkLibrary.getReserve(liquidity, address(this));
    ...
    */
    ...
    /**
     * @notice Allows governance to change the K3perNetworkHelper for max spend
     * @param _kprh new helper address to set
     */
    function setK3perNetworkHelper(IK3perNetworkHelper _kprh) external {
        require(msg.sender == governance, "setK3perNetworkHelper: !gov");
        KPRH = _kprh;
    }
